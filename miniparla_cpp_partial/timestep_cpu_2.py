...
def main(N, d, steps, NUM_WORKERS, cpu_array, sync_flag, vcu_flag,
    ...
    async def main_task():
        ...
        for t in range(steps):
            ...
            if dep_flag == 2:
                deps_chain = [T[1, t-1, i] for i in range(NUM_WORKERS)]

            for ng in range(NUM_WORKERS):
                if (dep_flag == 0) or (t == 0):
                    deps = []
                elif dep_flag == 1:
                    deps = [T[1, t-1, ng]]
                elif dep_flag == 2:
                    deps = deps_chain
                    deps = [T[1, t-1, i] for i in range(NUM_WORKERS)]
                else:
                    raise ValueError("Invalid dep_flag")
