...
cdef extern from "cpp_runtime.hpp" nogil:
    ctypedef void (*callerfunc)(void* f, void* python_task);
    ctypedef void (*stopfunc)(void* f);
    ctypedef void* launcherfunc;
    
    void launch_task_callback(callerfunc func, void* f, void* python_task) except +
...
cdef void callback_add(void* python_function, void* python_input) nogil:
    with gil:
        #print("Inside callback to cython", flush=True)
        task = <object>python_input
        scheduler = <object>python_function
        scheduler.cpp_callback(task)
...
cdef class PyInnerTask:
    ...
    def __cinit__(self):
        ...
        self.task = new InnerTask()

    def __init__(self, long id, object python_task, float vcus):
        self.task.id = id 
        self.task.set_task(<void*> python_task)
        self.task.vcus = vcus
...
class WorkerThread(ControllableThread, SchedulerContext):
    ...
    def _remove_task(self):
        with self._monitor:
            ...
            #print("Trying to free resources", flush=True)
            self.scheduler.incr_resources(self.task.req.vcus)
            #print("Finished free resources", flush=True)
            self.task = None
...
cdef class PyInnerScheduler:
    ...
    def __init__(self, int num_threads, float max_resources):
        self.scheduler.set_resources(max_resources)
...
class Scheduler(ControllableThread, SchedulerContext):
    ...
    def cpp_callback(self, task):
        #print("INSIDE CPP LAUNCHING CALLBACK", task._taskid, flush=True)
        success = False
        with self._thread_queue_monitor:
            #print("INSIDE CPP LAUNCHING CALLBACK (past lock)", flush=True)
            try:
                #print("ASSIGNING TASK", flush=True)
                #print("NUM THREADS", len(self._free_worker_threads), flush=True)
                worker = self._free_worker_threads.pop()
                #print("Got worker", flush=True)
                worker.assign_task(task)
                #print("ASSIGNED TASK", flush=True)
                success = True
                

            except IndexError:
                print("NO FREE THREADS", flush=True)
